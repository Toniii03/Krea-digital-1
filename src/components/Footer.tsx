...
export default function Footer() {
  return (
    <footer id="contacto" className="bg-krea-surface/50 backdrop-blur-sm">
      <div className="max-w-7xl mx-auto px-6 py-16">
        <div className="grid md:grid-cols-3 gap-12 items-center">
          <div className="flex flex-col items-center md:items-start">
            <div className="relative w-20 h-20 flex items-center justify-center mb-4">
              <div className="absolute inset-0 border-2 border-krea-blue rotate-45 rounded-lg" />
              <div className="absolute inset-2 border-2 border-krea-gold rotate-45 rounded-lg" />
              <span className="relative text-krea-gold font-extrabold text-2xl">K</span>
            </div>
            <p className="text-krea-blue font-bold text-lg">Krea Digital</p>
          </div>
          ...
          <div className="space-y-4">
            <a href={`mailto:${CONTACT_EMAIL}`} className="flex items-center gap-3 text-krea-blue hover:text-krea-gold transition-colors justify-center md:justify-start">
              <Mail className="w-4 h-4 text-krea-gold" />
        ...
        <div className="max-w-7xl mx-auto px-6 py-5 flex flex-col md:flex-row items-center justify-between gap-4">
          <ul className="flex flex-wrap justify-center gap-6">
            {NAV_LINKS.map(({ label, href }) => (
              <li key={href}>
                <a href={href} className="text-krea-blue/60 text-xs font-medium hover:text-krea-gold transition-colors">
                  {label}
                </a>
              </li>
            ))}
